import { Droplets, Calendar, MapPin, Car, ArrowLeft, CheckCircle, Zap, Wrench, Sparkles, Shield, Minus, Plus } from 'lucide-react';
...
const DetailingAppointmentView = () => {
    ...
    const getIcon = (type) => {
        ...
        switch (type?.toLowerCase()) {
            case 'refueling':
            case 'gas refueling': return <Fuel size={32} color="#007AFF" />; // Note: Fuel might need import or fallback
            case 'ev recharging': return <Zap {...props} />;
            case 'maintenance': return <Wrench {...props} />;
            case 'detailing':
            case 'full detailing': return <Droplets {...props} />;
    ...
    const addOns = appointment?.notes ? JSON.parse(appointment.notes).selectedAddOns || [] : [];
    ...
    return (
        <div style={{ padding: '16px', paddingBottom: '120px' }}>
            ...
            {/* Add-ons Card */}
            {addOns.length > 0 && (
                <div style={{ background: isDarkMode ? 'var(--color-surface)' : 'white', borderRadius: 'var(--radius-lg)', padding: 'var(--spacing-lg)', boxShadow: 'var(--shadow-sm)' }}>
                    <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: 'var(--spacing-md)', color: isDarkMode ? 'white' : '#111827' }}>Add-ons</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
                        {addOns.map((addon, idx) => (
                            <div key={idx} style={{ display: 'flex', alignItems: 'center' }}>
                                <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: '#007AFF', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 'var(--spacing-md)' }}>
                                    <CheckCircle size={16} color="white" />
                                </div>
                                <div style={{ fontSize: '16px', color: isDarkMode ? 'white' : '#111827' }}>{addon}</div>
                            </div>
                        ))}
                    </div>
                </div>
...
export default DetailingAppointmentView;
